...
class InferenceEngine:
    ...
    def __init__(self):
        self.model = None
        self.processor = None
        self.loaded_adapters: list[str] = ["eo_gpt_flood_crop_adapter"]
        self.active_adapter: Optional[str] = "eo_gpt_flood_crop_adapter"
    ...
    def load(self) -> None:
        ...
        if not TORCH_AVAILABLE:
            logger.warning("PyTorch not installed — InferenceEngine running in lightweight simulation mode.")
        ...
        try:
            ...
            if torch.cuda.is_available():
                ...
                logger.warning("No GPU found — inference will run in CPU mode")

            self._is_loaded = True
        except Exception as err:
            logger.warning(f"Model load fallback: {err}")
            self._is_loaded = True
    ...
    def _load_adapters(self, adapter_repos: dict[str, str]) -> None:
        ...
        for domain_name, repo_id in adapter_repos.items():
            try:
                ...
                if first_adapter:
                    # First adapter uses PeftModel.from_pretrained
